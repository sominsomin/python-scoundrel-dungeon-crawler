...
class Game:
    ...
    def draw_player_state(self) -> None:
        ...
        if self.player.weapon is not None:
            print(
                self.term.yellow(
                    (
                        f"Weapon: {self.player.weapon.value}, defeated creatures stack: {[creature.value for creature in self.player.weapon.defeated_creatures]}"
                    )
                )
            )
    ...
    def handle_card_interaction(
        self, selected_card: Card, selected_interaction: CardInteractionTypes
    ) -> None:
    ...
    def win_screen(self):
        ...
        print(
            self.term.black_on_green(
                self.term.center("Congratulations! You finished the dungeon!")
            )
        )
        print((self.term.center("Press any key to try again! ")))
        time.sleep(1)
    ...
    def check_term_window_size(self) -> None:
        if self.term.width < MIN_WIDTH or self.term.height < MIN_HEIGHT:
            ...
            print(
                self.term.center(
                    self.term.white_on_black(
                        f"current width/height: {self.term.width}/{self.term.height}, min swidth/height: {MIN_WIDTH}/{MIN_HEIGHT}"
                    )
                )
            )

    def handle_player_input(
        self, key, n_options, cursor_position_horizontal, cursor_position_vertical
    ) -> None:
    ...
    def draw_state(self, cursor_position_horizontal, cursor_position_vertical=0) -> int:
        ...
        for i, card in enumerate(self.drawn_cards):
            ...
            if card.is_creature():
                ...
                card_interactions = card.interactions()
            if (
                i == cursor_position_horizontal
                and len(card_interactions) <= cursor_position_vertical
            ):
        ...
        self.handle_player_input(
            key, n_options, cursor_position_horizontal, cursor_position_vertical
        )
    ...
    def run(self) -> None:
        print(
            self.term.clear + self.term.home + self.term.move_y(self.term.height // 2)
        )
        print(
            self.term.black_on_darkkhaki(
                self.term.center(
                    "You start into the dungeon! press any key to continue"
                )
            )
        )
    ...
    def fight_creature(
        self, creature: Card, selected_interaction: CardInteractionTypes
    ) -> None:
        ignore_weapon = (
            selected_interaction == CardInteractionTypes.FIGHT_CREATURE_BARE_HANDED
        )
